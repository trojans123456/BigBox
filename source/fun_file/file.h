#ifndef __FILE_H
#define __FILE_H

#include <stdio.h>
#include <stdint.h>
#include <limits.h>

#ifndef __WIN32__
#include <stdbool.h>
#include <sys/uio.h>
#endif


#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief The file_open_mode enum
 */
typedef enum file_open_mode {
    F_RDONLY,
    F_WRONLY,
    F_RDWR,
    F_CREATE,
    F_WRCLEAR, /* w+ content will be clear */
    F_APPEND,
} file_open_mode_t;

typedef enum file_type {
    F_NORMAL,
    F_DIR,
    F_LINK,
    F_SOCKET,
    F_DEVICE,

} file_type_t;

struct file_desc {
    union {
        int fd;
        FILE *fp;
    };
    char *name;
};

typedef struct file {
    struct file_desc *fd;
    const struct file_ops *ops;
    uint64_t size;
} file;

typedef struct file_info {
    uint64_t modify_sec;
    uint64_t access_sec;
    enum file_type type;
    char path[PATH_MAX];
    uint64_t size;
} file_info;

typedef struct file_systat {
    uint64_t size_total;
    uint64_t size_avail;
    uint64_t size_free;
    char fs_type_name[32];
} file_systat;

typedef struct file_ops {
    struct file_desc * (*open)(const char *path, file_open_mode_t mode);
    ssize_t (*write)(struct file_desc *fd, const void *buf, size_t count);
    ssize_t (*read)(struct file_desc *fd, void *buf, size_t count);
    off_t (*seek)(struct file_desc *fd, off_t offset, int whence);
    int (*sync)(struct file_desc *fd);
    size_t (*size)(struct file_desc *fd);
    void (*close)(struct file_desc *fd);
} file_ops_t;

/** 后端类型，io = linux syscall open  fio = stdlibc fopen */
typedef enum file_backend_type {
    FILE_BACKEND_IO,
    FILE_BACKEND_FIO,
} file_backend_type;

/**
 * @brief 设置类型
 * @param type
 */
void file_backend(file_backend_type type);

/**
 * @brief 仅仅创建一个文件
 * @param path
 * @return 0-succe -1=fail
 */
int file_create(const char *path);

/**
 * @brief 删除一个文件
 * @param path
 * @return
 */
int file_delete(const char *path);

/**
 * @brief 判断文件是否存在
 * @param path
 * @return
 */
bool file_exist(const char *path);

/**
 * @brief 打开一个文件
 * @param path
 * @param mode
 * @return
 */
struct file *file_open(const char *path, file_open_mode_t mode);

/**
 * @brief 关闭一个文件
 * @param file
 */
void file_close(struct file *file);

/**
 * @brief 从file读取数据到data中
 * @param file
 * @param data
 * @param size
 * @return
 */
ssize_t file_read(struct file *file, void *data, size_t size);

/**
 * @brief 从路径文件读取数据
 * @param path
 * @param data
 * @param size
 * @return
 */
ssize_t file_read_path(const char *path, void *data, size_t size);

/**
 * @brief file_write
 * @param file
 * @param data
 * @param size
 * @return
 */
ssize_t file_write(struct file *file, const void *data, size_t size);

ssize_t file_write_path(const char *path, const void *data, size_t size);

/**
 * @brief 文件大小
 * @param file
 * @return
 */
ssize_t file_size(struct file *file);
ssize_t file_get_size(const char *path);

/**
 * @brief 获取文件信息
 * @param path
 * @param info
 * @return
 */
int file_get_info(const char *path, struct file_info *info);


struct iovec *file_dump(const char *path);

int file_sync(struct file *file);

/**
 * @brief 设置文件指针位置
 * @param file
 * @param offset
 * @param whence
 * @return
 */
off_t file_seek(struct file *file, off_t offset, int whence);


int file_rename(const char* old_file, const char* new_file);

struct file_systat *file_get_systat(const char *path);

char *file_path_pwd();

/**
 * @brief 去除文件目录和后缀 basename
 * @param path
 * @return
 */
char *file_path_suffix(char *path);

/**
 * @brief 只保留目录 dirname
 * @param path
 * @return
 */
char *file_path_prefix(char *path);

int file_dir_create(const char *path);

int file_dir_remove(const char *path);

int file_dir_tree(const char *path);

/**
 * @brief dir大小
 * @param path
 * @param size
 * @return
 */
int file_dir_size(const char *path, uint64_t *size);

/**
 * @brief path下除了. .. 以外的文件夹个数
 * @param path
 * @return
 */
int file_num_in_dir(const char *path);

#ifdef __cplusplus
}
#endif

#endif
