...
/********* global ************/
static sel_signal_handler_t user_sighup_handler = NULL;
static sel_signal_handler_t user_sigint_handler = NULL;
...
int sel_stop_timer(sel_timer_t *timer)
{
    ...
    remove_from_heap(&(timer->sel_owner->timer_top),
             &(timer->sel_owner->timer_last),
             timer);
    timer->in_heap = 0;
    return 0;
}
