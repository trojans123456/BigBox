...
int add_header_message(http_request_msg_t *header,const char *key,const char *value)
{
    ...
    while(key[i] && i < sizeof(header->request_buf))
    {
    ...
    i = 0;
    while(value[i] && i < sizeof(header->request_buf))
    {
...
int init_post_header(http_post_request_t *header,char *message)
{
    ...
    while(method[i])
    {
        header->request.request_buf[header->offset++] = method[i];
        i++;
    }
...
{
    char buffer[128] = "";
    sprintf(buffer,"form-data; name=\"file\"; filename=\"%s\"",filename);
    add_header_message(&request->request,"Content-Disposition",buffer);
    ...
    {
        read_len = fread(value,sizeof(value),sizeof(char),fp);
        if(read_len > 0)
        {
