...
int recv_packet(int fd, char *buffer, struct sockaddr* addr, int *slen){
	int ret = recvfrom(fd, buffer, BUF_SIZE, 0 , addr, slen);
	#if 1
    int pos = 0;
...
packet pckt_struct;
packet *pckt = &pckt_struct;
char filename_buffer[128] = "";
char err_message_buffer[32] = "";
char data_buffer_buffer[1024] = "";
//pckt = (packet *) malloc(sizeof(packet));
init_packet(pckt,filename_buffer,err_message_buffer,data_buffer_buffer);
...
if(is_read_request(opcode) || is_write_request(opcode)){
    ...
    if(is_read_request(opcode)){
        ...
        expected_blck_num = 1;
    	request_package(opcode,temp_filename,mode,send_buffer);		//Raw header now in buffer
       ...
       do{
            ...
            if(!parser_raw_packet(recv_buffer, reclength, pckt)){
                error_package(TFTP_ERR_ILLEGAL_OPERATION, errmsgs[TFTP_ERR_ILLEGAL_OPERATION], send_buffer);
        ...
        if(err_blck_tries == 0 || timeout_tries == 0){
            error_package(TFTP_ERR_NOT_DEFINED, "Maximum Tries reached",send_buffer);
            send_packet(me, send_buffer, BUF_SIZE, otr, other_addrlen );
            finish(NULL, me);
        ...
        if(fp == NULL){
            rv1 = find_error();
            error_package(rv1 ,errmsgs[rv1], send_buffer );
            send_packet( me, send_buffer, BUF_SIZE, otr , other_addrlen );
            finish(NULL, me);
            perror("fopen");
            exit(1);
        ...
        ack_package(expected_blck_num, send_buffer);
    	...
    	while(data_size == 512){
            ...
            if(dest_tid != get_in_port(&other_addr)){
                ...
                error_package(TFTP_ERR_UNKNOWN_TID, errmsgs[TFTP_ERR_UNKNOWN_TID], recv_buffer);
    		...
    		if(!is_data_packet(pckt->opcode)){
	    		if(is_error_packet(pckt->opcode)){
                    if(pckt->error_code == TFTP_ERR_UNKNOWN_TID){
                        //Unknown tid error
    	...
    	ack_package(pckt->block_number, send_buffer);
    	if(send_packet(me, send_buffer,4, otr , other_addrlen ) == -1){
            finish(fp, me);
        ...
        err_blck_tries = MAX_TRIES;
    	request_package(opcode, temp_filename, mode, send_buffer);
          do{
        ...
        expected_blck_num++;
        data_package(expected_blck_num,recv_buffer, data_size, send_buffer);          //copy data header to buffer
        rv=0;
        while(data_size > 0){
            ...
            if( pckt->block_number == expected_blck_num ){
                ...
                data_package(expected_blck_num,recv_buffer, data_size, send_buffer);
            }
            else{
                printf("%d received %d expected\n", pckt->block_number , expected_blck_num);
            }
...
int main(int argc, char **argv){
	...
	if(!strcmp( argv[1], "-ra" )){
		type = TFTP_CMD_RRQ;
		mode = TFTP_ASCII_NUMBER;
	}
	else if(!strcmp( argv[1], "-ro" )){
		type = TFTP_CMD_RRQ;
		mode = TFTP_OCTET_NUMBER;
	}
