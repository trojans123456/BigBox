...
1. 设备描述符 (Device Descriptors)
    仅有一个;里面定义了该设备有多少种配置,每种配置 对应一个配置描述符
2. 配置描述符 (Configuration Descriptors)
    有多个;里面定义了有多少个接口;每个接口对应一个接口描述符
...
struct libusb_device_descriptor {
	 ...
	 * context. */
        uint8_t  bDescriptorType; // 设备描述符类型 固定为 LIBUSB_DT_DEVICE

	/** USB specification release number in binary-coded decimal. A value of
	 * 0x0200 indicates USB 2.0, 0x0110 indicates USB 1.1, etc. */
        uint16_t bcdUSB; //usb规范的版本号,表示本设备能使用于哪种协议,如2.0 = 0200 1.1 = 0110

	/** USB-IF class code for the device. See \ref libusb_class_code. */
        uint8_t  bDeviceClass; //类型代码由USB指定;0-表示所有接口都配置在描述符里，所有接口是独立的
        //1~0xFE 表示不同的接口关联的  0xFF-厂商自己定义

	/** USB-IF subclass code for the device, qualified by the bDeviceClass
	 * value */
        uint8_t  bDeviceSubClass; //子类型代码 如果bDeviceClass = 0 该字段一定为0
	...
	/** Maximum packet size for endpoint 0 */
        uint8_t  bMaxPacketSize0; //端点0最大分组大小 (只有8,16,32,64有效)
	...
	/** USB-IF product ID */
        ...
        // 供应商ID和产品ID就可以加载不同的驱动

	/** Device release number in binary-coded decimal */
        uint16_t bcdDevice; //设备出厂编码

	/** Index of string descriptor describing manufacturer */
        uint8_t  iManufacturer; //厂商描述符字符串索引
	...
	/** Number of possible configurations */
        uint8_t  bNumConfigurations; //可能的配置数,指配置字符串的个数
};
...
struct libusb_endpoint_descriptor {
	 ...
	 * this context. */
        uint8_t  bDescriptorType; //接口描述符类型．固定为0x05 LIBUSB_DT_ENDPOINT．

	/** The address of the endpoint described by this descriptor. Bits 0:3 are
	 * the endpoint number. Bits 4:6 are reserved. Bit 7 indicates direction,
	 * see \ref libusb_endpoint_direction.
	 */
        uint8_t  bEndpointAddress; //USB设备的端点地址．Bit7，方向，对于控制端点可以忽略，1/0:IN/OUT．Bit6-4，保留．BIt3-0：端点号
	 ...
	 */
        uint8_t  bmAttributes;//端点属性．Bit7-2，保留．BIt1-0：00控制，01同步，02批量，03中断

	/** Maximum packet size this endpoint is capable of sending/receiving. */
        uint16_t wMaxPacketSize; //本端点接收或发送的最大信息包大小．

	/** Interval for polling endpoint for data transfers. */
        uint8_t  bInterval; //轮询数据传送端点的时间间隔．对于批量传送和控制传送的端点忽略．对于同步传送的端点，必须为１，对于中断传送的端点，范围为1-255
...
struct libusb_interface_descriptor {
	/** Size of this descriptor (in bytes) */
        uint8_t  bLength;//描述符大小．固定为0x09

	/** Descriptor type. Will have value
	 * \ref libusb_descriptor_type::LIBUSB_DT_INTERFACE LIBUSB_DT_INTERFACE
	 * in this context. */
        uint8_t  bDescriptorType; //接口描述符类型．固定为0x04 LIBUSB_DT_INTERFACE
	 ...
	 * endpoint). */
        uint8_t  bNumEndpoints; //使用的端点数目．端点０除外

	/** USB-IF class code for this interface. See \ref libusb_class_code. */
        uint8_t  bInterfaceClass; //类型代码（由USB分配）

	/** USB-IF subclass code for this interface, qualified by the
	 * bInterfaceClass value */
        uint8_t  bInterfaceSubClass; //子类型代码（由USB分配)
 ...
 * All multiple-byte fields are represented in host-endian format.
 */
/** 配置描述符 定义了设备的配置信息,一个设备可以有多个配置描述符*/
struct libusb_config_descriptor {
	 ...
	 * in this context. */
        uint8_t  bDescriptorType;//类型 LIBUSB_DT_CONFIG

	/** Total length of data returned for this configuration */
        uint16_t wTotalLength;//返回整个数据的长度,指此配置返回的配置描述符，接口描述符以及端点描述符的全部大小

	/** Number of interfaces supported by this configuration */
        uint8_t  bNumInterfaces; //配置所支持的接口数．指该配置配备的接口数量，也表示该配置下接口描述符数量

	/** Identifier value for this configuration */
        uint8_t  bConfigurationValue; //作为Set Configuration的一个参数选择配置值

	/** Index of string descriptor describing this configuration */
        uint8_t  iConfiguration;// 用于描述该配置字符串描述符的索引
	 ...
	 * of 8 mA when the device is operating in super-speed mode. */
        uint8_t  MaxPower;//总线供电的USB设备的最大消耗电流．以2mA为单位
