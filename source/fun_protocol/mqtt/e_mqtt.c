...
ssize_t mqtt_unpack_fixed_header(struct mqtt_response *response, const uint8_t *buf, size_t bufsz) {
    ...
    fixed_header->control_type  = *buf >> 4; /* 解析 报文类型 高4位 */
    fixed_header->control_flags = *buf & 0x0F; /*标志 低4位*/
...
{
    uint8_t *curdata = buf;// skip fixed header 2byte
    struct mqtt_response_connect *connect = &mqtt_response->decoded.connect;

    unsigned int remain = mqtt_response->fixed_header.remaining_length;
    printf("remain = %d  curdata = %x %x\n",remain,*curdata,*(curdata + 1));

    unsigned short protocol_len = (*curdata << 8 | *(curdata + 1));
    connect->protocol_name.name_len = protocol_len;
    printf("--protocol_len = %d \n",connect->protocol_name.name_len);

    curdata += 2;

    memcpy(connect->protocol_name.protocol_name,curdata,protocol_len);

#if 0

    int i = 0;
    for(i = 0;i < sizeof(connect->protocol_name.protocol_name);i++)
    {
        printf("--%c-- \n",connect->protocol_name.protocol_name[i]);
    }

#endif
    curdata += protocol_len;

    connect->protocol_level = *curdata;

    printf("-- level = %d \n",connect->protocol_level);

    curdata += 1;//sizeof(connect->protocol_level);

    printf("-- connect_flag = %x \n",*curdata);
    connect->connect_flag.clean_session = (*curdata >> 1) & 0x01;
    connect->connect_flag.will_flag = (*curdata >> 2) & 0x01;
    connect->connect_flag.will_qos = (*curdata >> 3) & 0x03;
    connect->connect_flag.will_retain = (*curdata >> 5) & 0x01;
    connect->connect_flag.password_flag = (*curdata >> 6) & 0x01;
    connect->connect_flag.username_flag = (*curdata >> 7) & 0x01;

    curdata += 1;//sizeof(connect->connect_flag);
    ...
    curdata += sizeof(connect->keep_alive);

    connect->client_id.string_len = (*curdata << 8 | *(curdata + 1));
    connect->client_id.str = curdata + sizeof(connect->client_id.string_len);
    printf("str = %s str_len = %d \n",connect->client_id.str,connect->client_id.string_len);

    curdata += sizeof(connect->client_id.string_len) + connect->client_id.string_len;

    if(connect->connect_flag.will_flag == 1)
    {
        connect->will_topic.string_len = (*curdata << 8 | *(curdata + 1));
        connect->will_topic.str = curdata + sizeof(connect->will_topic.string_len);

        printf("--will_topic = %s \n",connect->will_topic.str);

        curdata += sizeof(connect->will_topic.string_len) + connect->will_topic.string_len;

        connect->will_msg.string_len = (*curdata << 8 | *(curdata + 1));
        connect->will_msg.str = curdata + sizeof(connect->will_msg.string_len);

        printf("--will_msg = %s \n",connect->will_msg.str);

        curdata += sizeof(connect->will_msg.string_len) + connect->will_msg.string_len;
    }

    if(connect->connect_flag.password_flag == 1)
    {
        connect->password.string_len = (*curdata << 8 | *(curdata + 1));
        connect->password.str = curdata + sizeof(connect->password.string_len);

        printf("--password =%s \n",connect->password.str);

        curdata += sizeof(connect->password.string_len) + connect->password.string_len;
    }

    if(connect->connect_flag.username_flag == 1)
    {
        connect->user_name.string_len = (*curdata << 8 | *(curdata + 1));
        connect->user_name.str = curdata + sizeof(connect->user_name.string_len);

        printf("--username = %s \n",connect->user_name.str);

        curdata += sizeof(connect->user_name.string_len) + connect->user_name.string_len;
    }
...
{
   uint8_t *curdata = buf;// skip fixed header 2byte
    struct mqtt_response_subscribe *subscribe = &mqtt_response->decoded.subscribe;

    unsigned int remain = mqtt_response->fixed_header.remaining_length;
    printf("remain = %d  curdata = %x %x\n",remain,*curdata,*(curdata + 1));

    //VAR HEAD
    unsigned short var_head = (*curdata << 8 | *(curdata + 1));
    subscribe->varhead.varhead = var_head;
    printf("--var_head = %x \n",subscribe->varhead.varhead);

    curdata += sizeof(unsigned short); // + 2

    //payload
    remain -= 2; //skip var head

    unsigned short topic_len = 0;
    unsigned int pos = 0;
    unsigned int cur_len = 0;
    while(cur_len < remain)
    {
        topic_len = (*curdata << 8 | *(curdata + 1));
        curdata += sizeof(topic_len); // +2

        subscribe->payloads[pos].topic.str = curdata;
        curdata += topic_len;
        printf("--subscribe = %s \n",subscribe->payloads[pos].topic.str);

        subscribe->payloads[pos].qos = *curdata;
        printf("--qos = %d \n",subscribe->payloads[pos].qos);

        curdata += 1; // qos  + 1byte

        pos++;
        cur_len += sizeof(topic_len) + topic_len + 1;
    }
}
...
void mqtt_mq_clean(struct mqtt_message_queue *mq) {
    ...
    for(new_head = mqtt_mq_get(mq, 0); new_head >= mq->queue_tail; --new_head) {
        ...
        /* 可选 清除上次的connect*/
        if(new_head->state == MQTT_QUEUED_AWAITING_ACK && new_head ->control_type == MQTT_CONTROL_CONNECT)
            new_head->state = MQTT_QUEUED_COMPLETE;
...
ssize_t mqtt_unpack_response(struct mqtt_response* response, const uint8_t *buf, size_t bufsz) {
    ...
    switch(response->fixed_header.control_type) {
        case MQTT_CONTROL_CONNECT:
            printf("-------connect-----------\n");
            mqtt_unpack_connect_response(response,buf); // i don't know why rv == 3
            break;
        case MQTT_CONTROL_CONNACK:
        ...
        case MQTT_CONTROL_SUBSCRIBE:
            printf("---subscribe--\n");
            mqtt_unpack_subscribe_response(response,buf);
            break;
        case MQTT_CONTROL_SUBACK:
