...
static void on_recv(union sigval sv)
{
    ...
    len = _mq_recv(ipc, ctx->_mq_recv_buf, MAX_IPC_MESSAGE_SIZE);
    if (len == -1) {
        printf("_mq_recv failed!\n");
        return;
    }
    if (ctx->_mq_recv_cb) {
        ctx->_mq_recv_cb(ipc, ctx->_mq_recv_buf, len);
    } else {
...
static void *_mq_init(struct ipc *ipc, uint16_t port, enum ipc_role role)
{
    ...
    ctx->_mq_recv_buf = calloc(1, MAX_IPC_MESSAGE_SIZE);
    if (!ctx->_mq_recv_buf) {
        printf("malloc failed!\n");
        return NULL;
    }
