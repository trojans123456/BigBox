 ...
 * @_old : the element to be replaced
...
#define list_for_each_entry(pos, head, member, type) \
    for (pos = list_first_entry(head, type, member); &pos->member != (head); pos = list_next_entry(pos, member, type))
...
#define list_for_each_entry_reverse(pos, head, member, type) \
    for (pos = list_last_entry(head, type, member); &pos->member != (head); pos = list_prev_entry(pos, member, type))
...
#define list_for_each_entry_continue(pos, head, member, type) \
    for (pos = list_next_entry(pos, member, type); &pos->member != (head); pos = list_next_entry(pos, member, type))
...
#define list_for_each_entry_continue_reverse(pos, head, member, type) \
    for (pos = list_prev_entry(pos, member, type); &pos->member != (head); pos = list_prev_entry(pos, member, type))
...
#define list_for_each_entry_from(pos, head, member, type) \
    for (; &pos->member != (head); pos = list_next_entry(pos, member, type))
...
#define list_for_each_entry_safe(pos, n, head, member, type)                                                         \
    for (pos = list_first_entry(head, type, member), n = list_next_entry(pos, member, type); &pos->member != (head); \
...
#define list_for_each_entry_safe_continue(pos, n, head, member, type)                                              \
    for (pos = list_next_entry(pos, member, type), n = list_next_entry(pos, member, type); &pos->member != (head); \
         pos = n, n = list_next_entry(n, member, type))
...
#define list_for_each_entry_safe_from(pos, n, head, member, type) \
    for (n = list_next_entry(pos, member, type); &pos->member != (head); pos = n, n = list_next_entry(n, member, type))
...
#define list_for_each_entry_safe_reverse(pos, n, head, member, type)                                                \
    for (pos = list_last_entry(head, type, member), n = list_prev_entry(pos, member, type); &pos->member != (head); \
         pos = n, n = list_prev_entry(n, member, type))
 ...
 */
#define list_safe_reset_next(pos, n, member, type) n = list_next_entry(pos, member, type)
 ...
 */
static inline void hlist_move_list(struct hlist_head *_old, struct hlist_head *_new)
{
...
#define hlist_for_each_entry_safe(pos, n, head, member, type)       \
    for (pos = hlist_entry_safe((head)->first, type, member); pos && ({n = pos->member.next; true;}); pos = hlist_entry_safe(n, type, member))
