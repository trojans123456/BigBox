...
/**
 * @brief snd_pcm_open 打开一个PCM
 * @param pcm  返回的PCM句柄
 * @param name 句柄标识 ascii
 * @param stream
 * @param mode  SND_PCM_NONBLOCK等等
 * @return
 */
int snd_pcm_open(snd_pcm_t **pcm, const char *name, 
		 snd_pcm_stream_t stream, int mode);
/**
 * @brief snd_pcm_open_lconf 使用本地配置打开PCM
 * @param pcm
 * @param name
 * @param stream
 * @param mode
...
int snd_pcm_open_lconf(snd_pcm_t **pcm, const char *name, 
		       ...
		       snd_config_t *lconf);
/**
 * @brief snd_pcm_open_fallback 打开一个备用的PCM
 * @param pcm
 * @param root
 * @param name
 * @param orig_name  原始名称
 * @param stream
 * @param mode
 * @return
 */
